...
import { readableStreamToString, HelcimError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new HelcimError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new HelcimError(e, e.response?.data)
                }
                if (innerError instanceof HelcimError) {
                    // Got "HelcimError" from the above try block
                    throw innerError;
                }
